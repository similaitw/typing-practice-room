...
- 自訂文章最多 10,000 字元。
- 班級＋姓名＋座號身分與資料庫排行榜；公開排行榜只顯示目前啟用的雲端學生。
...
原 Roadmap 的 Phase 10「正向成就系統」與 Phase 11「班級挑戰」目前依使用者決定暫緩。下一輪若不做遊戲化，優先處理既有技術待辦與 Production E2E，例如部署公開排行榜停用學生修正、學生／教師完整真實流程驗證，以及必要的安全性與可維護性整理。
