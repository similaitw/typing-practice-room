...
async function ensureAssignmentSchema(sql) {
  ...
  schemaPromise = (async () => {
    ...
    await sql`ALTER TABLE typing_records ADD COLUMN IF NOT EXISTS mistakes jsonb`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS typing_progress (
      ...
      best_speed integer NOT NULL DEFAULT 0 CHECK (best_speed BETWEEN 0 AND 10000),
      attempts integer NOT NULL DEFAULT 0 CHECK (attempts >= 0),
      completed_at timestamptz,
      updated_at timestamptz NOT NULL DEFAULT now(),
      PRIMARY KEY (student_id, lesson_id)
    )`;
    await sql`CREATE INDEX IF NOT EXISTS typing_progress_student_updated_idx
      ON typing_progress (student_id, updated_at DESC)`;
  })().catch(error => {
    schemaPromise = null;
    throw error;
  });
